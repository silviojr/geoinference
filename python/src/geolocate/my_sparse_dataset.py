...
class SparseDataset(object):
	...
	def __init(self, settings_file="dataset.json",
		...
		for line in open(self._users_with_locations_fname):
			...
			lat = float(lat)
			lon = float(lon)
			self._users_real_locations[user_id] = (lat, lon)
	...
	def user_iter(self):
		"""
		Return an iterator over all fake posts in the dataset grouped by user. Each
	...
	def user_home_location_iter(self):
		...
		for user in self._users_real_locations:
			if user in self.excluded_users:
				continue
			yield (user, self._users_real_locations[user])
